...
EXTRA_OEMAKE = "-C ${B}/addons/pvr.vdr.vnsi"

RDEPENDS_${PN} = "xbmc"
do_install_append() {
    mkdir -p "${D}${libdir}"/kodi/addons
    mv "${D}${libdir}"/pvr.vdr.vnsi "${D}${libdir}"/kodi/addons/
    mkdir -p "${D}${datadir}"/kodi/addons
    mv "${D}${datadir}"/pvr.vdr.vnsi "${D}${datadir}"/kodi/addons/
    install -m 0644 "${B}"/addons/pvr.vdr.vnsi/addon/addon.xml "${D}${datadir}"/kodi/addons/pvr.vdr.vnsi/
}
